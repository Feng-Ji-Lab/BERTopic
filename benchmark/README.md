...
The optional results-legacy-0.17.4/ directory is generated by the earlier single-process prototype. Generated result directories are ignored by Git and must not be used as the final D2 results.
...
The lightweight safetensors restoration check verifies topic metadata. The additional full pickle round trip verifies cached assignments/strengths and post-load predictions at 1e-12. The script stops on failed operations or restoration checks. A manifest records the archive/script and generated artifact hashes. The complete output directory, including model.pkl, is ignored by Git and can be recreated with this script.

Build or download the exact tagged source archive before running the example, then pass its path with --package-archive. Generated package archives under provenance/releases/ are ignored by Git. Full topic metadata contain list columns; the example CSV serializer preserves these as JSON-valued cells. Its regression case can be run with:

~~~powershell
Rscript benchmark/test_example_io.R
~~~

The validated Windows runs established the 0.1.2 manuscript results. Their results/, results-windows-0.1.1/, and example-results/ directories are generated locally and ignored by Git.

The package check reported 159 passes, no failures/warnings/skips, and one existing UTF-8 data NOTE. The source tag is public on GitHub; DOI and E remain pending.

After regenerating the complete outputs, validate them using Python 3.10+ without loading machine-learning packages: python benchmark/verify_archive.py. Refresh the ignored archive_sha256.json with --write only after checking the execution manifests.

Publication ZIP builder, offline Git bundle support, Windows reproduction script and author/Zenodo drafts are maintained in the sibling bert repository. See ../bert/publication/README.md for the remaining DOI and author-confirmation steps.

The revised paper source is maintained in the sibling ../bert/ repository. Its scripts/sync_manuscript.py reads the ignored benchmark outputs from this repository and preserves the edited narrative.
