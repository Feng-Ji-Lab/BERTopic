...
In its current form, \pkg{BERTopic} provides an R-oriented access layer and preserves the selected outputs tested here. The contribution is interface design, workflow documentation, and bounded input--output validation. \todo{Before submission, supply the archive DOI, and complete outstanding dataset-provenance and author-information fields.}
...
\textbf{Code availability.} Development source is at \url{https://github.com/Feng-Ji-Lab/BERTopic}. The benchmark used version 0.1.2, tag \fcode{v0.1.2}, commit \fcode{faee106}, and the retained source archive checksum. \todo{Insert the published version-specific archive DOI for the exact source and analysis scripts.}
