...
\begin{lstlisting}[language={},float=htbp,caption={Creating the recorded Python environment from PowerShell.}]
conda env create --name r-bertopic-paper-012 `
  --file provenance/windows-bertopic-0.16.0.yml
\end{lstlisting}

If required R dependencies are absent, run \fcode{Rscript benchmark/publication/install_r_dependencies.R} from the analysis checkout. This bootstrapper uses current CRAN versions, not a historical dependency lock. Then install the retained source archive and bind the named environment before Python is initialized:

\begin{lstlisting}[float=htbp,caption={Installing the tested R package and binding its Python backend.}]
...
After completing the one-time setup in Section~\ref{sec:installation}, start a fresh R session and bind its named environment before checking availability or importing Python modules. The following code loads the data, records the backend, and fixes UMAP's random state. Setting only the global R or NumPy seed is not sufficient to make every stochastic component deterministic.

\begin{lstlisting}[float=htbp,caption={Checking the backend and fitting a BERTopic model from R.}]
library(BERTopic)
library(reticulate)
...
\subsection{Visualization and representation updates}
...
\begin{lstlisting}[caption={Creating HTML visualizations and updating topic representations.}]
...
\subsection{Persistence and restoration}

Choose the save format according to the intended reuse. Safetensors stores lightweight topic information and representations, omitting the fitted reducer and clusterer. With the default \fcode{save_embedding_model = FALSE}, it also omits an embedding-model reference. The following check compares restored topic metadata; it does not test new-document predictions.

\begin{lstlisting}[float=htbp,caption={Saving lightweight topic information and checking restored metadata.}]
...
For reuse of the fitted clustering path, save a full pickle with \fcode{save_embedding_model = TRUE}. Compare predictions from the current model with those from the loaded model, using the same documents and environment. The comparison below is made after representation updates, so both calls use the same model state. Only load trusted pickle files.

\begin{lstlisting}[float=htbp,caption={Saving the complete model and checking post-load predictions.}]
