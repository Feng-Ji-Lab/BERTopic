...
- A1?API ?????????????????????????? BERTopic/Reviews/API_AUDIT.md?
- B1?B8???????? Windows ???????? lightweight save ??????pt ????????????
- C1?C2??? BERTopic 0.16.0???????????????C3 ???????? Windows?
- D1?0.1.2 ?? tag?????? SHA-256 ?????? release ?????
- D2?????? 0.1.2 ??????? R/Python benchmark ??? worked example????????????????
...
### [ ] C3. 在 macOS、Linux、Windows 的干净环境中运行完整测试矩阵
...
- [ ] 所有被点名的软件审计问题有修复 commit 和非 skip 的回归测试。
- [ ] 支持的 Python BERTopic 版本范围有三平台干净测试证据。
- [ ] Conda/virtualenv 安装结果与精确依赖版本可复现。
- [ ] SMS packaged data 可由公开源确定性重建。
- [ ] 论文所有结果来自同一个明确的新 package release。
- [ ] 原始输出、脚本、环境、seed、hash、process settings、图表均已归档。
